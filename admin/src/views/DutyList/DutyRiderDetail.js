...
import { Truck, Phone, Star, MapPin, ArrowLeft, CreditCard, Calendar, Award } from 'lucide-react';
...
const DutyRiderDetail = () => {
  ...
  useEffect(() => {
    if (!rider || !rider.location || !Array.isArray(rider.location.coordinates) || rider.location.coordinates.length < 2) {
      console.warn("Invalid or missing rider location. Using default coordinates.");
      return;
    }

    const latitude = rider.location.coordinates[1] ?? 28.6139; // Default: New Delhi
    const longitude = rider.location.coordinates[0] ?? 77.2090;
    ...
    L.marker([latitude, longitude]).addTo(map).bindPopup("Rider's Location").openPopup();
  ...
  const handleDocumentVerifyToggle = async (id, status) => {
    try {
      ...
      const res = await axios.put(`http://www.appapi.olyox.com/api/v1/rider/update_rider_document_verify/${id}`, {
                ...
                <div className="col-md-6">
                  <div className="mb-3">
                    ...
                    {rider?.location?.coordinates &&
                      ...
                      !isNaN(rider.location.coordinates[1]) &&
                      !isNaN(rider.location.coordinates[0]) ? (
                      <div>
...
export default DutyRiderDetail
