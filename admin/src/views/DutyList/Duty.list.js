...
const Dutyist = () => {
  ...
  const fetchRiders = async () => {
    ...
    try {
      ...
      const { data } = await axios.get('http://www.appapi.olyox.com/api/v1/rider');
  ...
  const handleStatusToggle = async (riderId, currentStatus) => {
    setLoading(true);
    try {
      await axios.put(`http://www.appapi.olyox.com/api/v1/rider/updateRiderBlock/${riderId}`, {
  ...
  const handleViewDetails = (riderId) => {
    navigate(`/Duty/duty-rider-detail/${riderId}`);
  };

  const heading = ['S.No', 'Rider Name', 'Rider Number', 'Vehicle Name', 'Vehicle Type', 'Total Rides', 'Rating', 'Actions'];
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <Table
          heading="Riders"
          btnText="All Riders Location"
          btnURL="/Duty/all-rider-location"
