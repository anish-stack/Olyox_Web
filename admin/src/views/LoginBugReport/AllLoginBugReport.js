...
const AllLoginBugReport = () => {
    ...
    const fetchBugReports = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('http://localhost:3100/api/v1/admin/reports');
    ...
    const deleteBugReport = async (id) => {
        setLoading(true);
        try {
            const res = await axios.delete(`http://localhost:3100/api/v1/admin/report/${id}`);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const { data } = await axios.put(`http://localhost:3100/api/v1/admin/report/${selectedReport._id}`, {
                status,
                bugFix,
            });
