...
const CabBooking = () => {
    ...
    const handleDelete = async (vendorId) => {
        try {
            const res = await axios.delete(`https://demoapi.olyox.com/api/v1/rides/delete_rider_ride/${vendorId}`);
    ...
    const handleUpdateOrderStatus = async (orderId, status) => {
        try {
            const res = await axios.put(`https://demoapi.olyox.com/api/v1/rides/update_rider_ride_status/${orderId}`, { status });
            toast.success(res.data.message);
