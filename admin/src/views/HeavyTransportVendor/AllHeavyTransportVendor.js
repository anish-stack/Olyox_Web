...
const AllHeavyTransportVendor = () => {
    ...
    const fetchVendors = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('https://demoapi.olyox.com/api/v1/heavy/get_all_hv_vendor');
    ...
    const handleStatusToggle = async (vendorId, currentStatus) => {
        setLoading(true);
        try {
            await axios.put(`https://demoapi.olyox.com/api/v1/heavy/update_hv_vendor_is_block_status/${vendorId}`, {
    ...
    const handleDelete = async(vendorId) => {
        try {
            const data = await axios.delete(`https://demoapi.olyox.com/api/v1/heavy/heavy_vehicle_profile_delete/${vendorId}`);
