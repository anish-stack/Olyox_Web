...
const HeavyTransportVendorDetail = () => {
  ...
  const fetchVendorDetails = async () => {
    ...
    try {
      const { data } = await axios.get(`https://www.appapi.olyox.com/api/v1/heavy/heavy-vehicle-profile/${id}`);
  ...
  const handleDocumentVerifiedToggle = async (id, isAlldocumentsVerified) => {
    ...
    try {
      const res = await axios.put(`https://www.appapi.olyox.com/api/v1/heavy/update_hv_vendor_document_verify/${id}`, {
