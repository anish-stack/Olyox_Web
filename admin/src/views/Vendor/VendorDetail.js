...
function VendorDetail() {
    ...
    const fetchWithdrawls = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get(`https://www.api.olyox.com/api/v1/admin-withdrawals?id=${id}`);
            setWithDrawals(data.
                withdrawal
            );
            console.log(data.
    ...
    const fetchRecharge = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get(`https://www.api.olyox.com/api/v1/get-all-admin-recharge?id=${id}`);
