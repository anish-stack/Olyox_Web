...
function AllCategory() {
    ...
    const handleUpdateActive = async (id, currentStatus) => {
        ...
        try {
            ...
            const res = await axios.put(`https://olyox.digital4now.in/api/v1/update_category_status/${id}`, {
    ...
    const handleDeleteBanner = async (id) => {
        setLoading(true);
        try {
            const res = await axios.delete(`https://olyox.digital4now.in/api/v1/categories/${id}`);
            toast.success(res?.data?.message);
            handleFetchBanner();
        } catch (error) {
