...
const AllUser = () => {
  ...
  const fetchUsers = async () => {
      setLoading(true);
      try {
          const { data } = await axios.get('http://www.appapi.olyox.com/api/v1/user/get_all_user');
  ...
  const handleStatusToggle = async (userId, currentStatus) => {
      setLoading(true);
      try {
          await axios.put(`http://www.appapi.olyox.com/api/v1/user/update_user_block/${userId}`, {
