...
const EditPrivacyPolicy = () => {
  ...
  const fetchPolicy = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`https://demoapi.olyox.com/api/v1/admin/policy/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`https://demoapi.olyox.com/api/v1/admin/policy/${id}`, formData);
