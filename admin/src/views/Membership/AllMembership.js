...
function AllMembership() {
    ...
    const handleUpdateActive = async (id, currentStatus) => {
        ...
        try {
            ...
            const res = await axios.put(`https://www.api.olyox.com/api/v1/update_membership_status/${id}`, {
    ...
    const handleDeleteMembershipPlan = async (id) => {
        setLoading(true);
        try {
            const res = await axios.delete(`https://www.api.olyox.com/api/v1/membership-plans/${id}`);
            toast.success(res?.data?.message);
            handleFetchMembershipPlans();
        } catch (error) {
