...
function AllMembership() {
    ...
    const handleFetchMembershipPlans = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('https://apiking.digital4now.in/api/v1/membership-plans');
    ...
    const handleUpdateActive = async (id, currentStatus) => {
        ...
        try {
            ...
            const res = await axios.put(`https://apiking.digital4now.in/api/v1/update_membership_status/${id}`, {
    ...
    const handleDeleteMembershipPlan = async (id) => {
        setLoading(true);
        try {
            const res = await axios.delete(`https://apiking.digital4now.in/api/v1/membership-plans/${id}`);
            toast.success(res?.data?.message);
            handleFetchMembershipPlans();
        } catch (error) {
