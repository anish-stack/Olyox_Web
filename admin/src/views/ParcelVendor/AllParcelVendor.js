...
const AllParcelVendor = () => {
    ...
    const fetchVendors = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('http://www.appapi.olyox.com/api/v1/parcel/get_all_parcel_user');
    ...
    const handleStatusToggle = async (vendorId, currentStatus) => {
        setLoading(true);
        try {
            await axios.put(`http://www.appapi.olyox.com/api/v1/parcel/update_parcel_is_block_status/${vendorId}`, {
