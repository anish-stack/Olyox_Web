...
function AllRecharge() {
    ...
    const fetchRecharges = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('api/v1/get-all-recharge');
    ...
    const togglePaymentApproval = async (id) => {
        ...
        try {
            ...
            const res = await axios.get(`api/v1/approve_recharge?_id=${id}`);
    ...
    const handleCancelRecharge = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.put(`api/v1/cancel_recharge?_id=${selectedRechargeId}`, {
                cancelReason,
